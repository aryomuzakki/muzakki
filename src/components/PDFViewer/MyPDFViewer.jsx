...
const defaultPDF =
  "/get-raw?filename=data/cv_muhammad_aryo_muzakki&format=dataurl";
...
const MyPDFViewer = ({ pathOrFileData, filename = "file.pdf" }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      document
        .getElementsByClassName("document-container")[0]
        .removeEventListener("keydown", zoomShortcut);

      document
        .getElementsByClassName("react-pdf__Document")[0]
        .removeEventListener("wheel", zoomWheel);
    };
  }, []);
  ...
  const onDocumentLoadSuccess = useCallback(({ numPages: nextNumPages }) => {
    ...
    document
      .getElementsByClassName("document-container")[0]
      .addEventListener("keydown", zoomShortcut);

    document
      .getElementsByClassName("react-pdf__Document")[0]
      .addEventListener("wheel", zoomWheel);
  });
  ...
  return (
    <div className="pdf-viewer-container tw-min-h-screen tw-bg-zinc-600">
      <header className="tw-fixed tw-z-50 tw-w-full">
        <div className="glass-bg-dark"></div>
        <div className="tw-z-50 tw-flex tw-h-14 tw-w-full tw-items-center tw-justify-between tw-overflow-x-scroll tw-p-2 tw-shadow-[0_1px_4px_rgba(0,0,0,0.3)] sm:tw-overflow-x-auto">
          <div className="tw-mr-4 tw-flex tw-w-full tw-items-center tw-space-x-4 tw-pl-4">
            {isOutlineOpen && (
              <button
                type="button"
                className="tw-du-btn tw-du-btn-circle tw-du-btn-ghost tw-du-btn-sm tw-h-10 tw-w-10"
                onClick={(ev) => {
                  toggleOutline();
                }}
              >
          ...
          <div className="tw-flex tw-flex-shrink-0 tw-items-center tw-justify-between tw-gap-8 tw-pr-4">
            <form
              className="tw-flex"
              onSubmit={(ev) => {
                ev.preventDefault();
                ...
                if (targetPageNum < 1) {
                  ev.target.pageNum.value =
                    pagesInViewState.findIndex((val) => val) + 1;
                } else {
                  const targetEl = document.querySelector(
                    `div[data-page-number="${targetPageNum}"]`,
                  );
            ...
            >
              <span>
                ...
                <input
                  type="text"
                  name="pageNum"
                  className="tw-du-input tw-du-input-sm tw-mx-2 tw-w-12 tw-rounded tw-bg-[#48484dbf] tw-text-center tw-backdrop-blur-[16px] tw-backdrop-saturate-[180%]"
            ...
            >
              <button
                ...
                className="tw-du-btn tw-du-btn-circle tw-du-btn-ghost tw-du-btn-sm tw-h-7 tw-min-h-0 tw-w-7"
              >
                {/* prettier-ignore */}
              ...
              <label className="tw-du-input tw-du-input-sm  tw-flex tw-items-center tw-gap-2 tw-rounded tw-bg-[#48484dbf] tw-backdrop-blur-[16px] tw-backdrop-saturate-[180%]">
                <input
                  type="text"
                  name="zoomPercent"
        ...
        {/*  */}
        <div
          className={`tw-absolute tw-right-16 tw-z-[3] tw-transition-all ${isSearchBarOpen ? "tw-top-16" : "-tw-top-20"}`}
        >
          <input
            type="text"
            name="searchQuery"
            ...
            ref={searchQueryBox}
            className="tw-du-input tw-du-input-sm tw-bg-[#1a1a1bbf] tw-shadow-[0_0_4px_0_#00000080] tw-backdrop-blur-[16px] tw-backdrop-saturate-[180%] focus:tw-outline-[#27272a4d]"
          />
        </div>
      </header>

      <div
        className="document-container tw-pt-14 focus:tw-outline-none"
        tabIndex={-1}
        ref={setContainerRef}
      >
        <Document
        ...
        >
          {}
          <div
            className={`outline-wrapper glass-bg-dark ${isOutlineOpen ? "tw-left-0" : "-tw-left-96"}`}
          >
